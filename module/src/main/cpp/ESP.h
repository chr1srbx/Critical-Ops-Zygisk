...
void DrawLine(ImVec2 start, ImVec2 end, ImVec4 color, int thickness, auto background) {
    if(background) {
        background->AddLine(start, end, ImColor(color.x,color.y,color.z,color.w), thickness);
    }
}

void DrawBox(ImVec4 rect, ImVec4 color, int thickness, auto background) {
...
void DrawBox2(float x, float y, float width, float height, ImColor color, int thickness, auto background)
{
    if(background) {
...
void DrawOutlinedBox2(float x, float y, float width, float height, ImVec4 color, int thickness, auto background)
{
...
void DrawFilledRect(float x, float y, float width, float height, ImVec4 color, auto background)
{
    if(background) {
...
void DrawOutlinedFilledRect(float x, float y, float width, float height, ImVec4 color, auto background)
{
...
void DrawBones(void* character, int bone1, int bone2, ESPCfg cfg, auto background){
    if(character != nullptr){
        ...
        if(wsbone1.Z > 0 && wsbone2.Z > 0){
            DrawLine(ImVec2(wsbone1.X, wsbone1.Y), ImVec2(wsbone2.X, wsbone2.Y), ImColor(cfg.boneColor.x, cfg.boneColor.y, cfg.boneColor.z), 3, background);
        }
    }
}

void DrawText(ImVec2 position, ImVec4 color, std::string text, ImFont* font, auto background) {
    if(background) {
...
void CoolCrosshair(ImVec2 position, ImVec4 color, auto background) {
