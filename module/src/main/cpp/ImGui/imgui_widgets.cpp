...
#pragma warning (disable : 4244) // checkbox
#pragma warning (disable : 4100) // colorpicker
#pragma warning (disable : 4189) // colorpicker
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImRect rect(pos - ImVec2(0, 10), pos + size);
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max + ImVec2(0, 0), ImColor(0.06f, 0.06f, 0.06f, 1.f), 3.f);
    window->DrawList->AddRectFilled(rect.Min, rect.Max + ImVec2(0, 0), ImColor(100, 108, 252, it_anim->second.active_rect_alpha), 3.f);
    window->DrawList->AddRect(rect.Min, rect.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 3.f);
    RenderTextClipped(rect.Min, rect.Max, label, NULL, &label_size, ImVec2(0.5f, 0.5f));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float size = GetWindowWidth() - 358;

    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos + ImVec2(0, 20);
    const ImRect rect(pos, pos + ImVec2(size, 60));
    const ImRect click_bb(rect.Max - ImVec2(60, 60), rect.Max);
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(click_bb, id, &hovered, &held);
    if (pressed)
    {
        *v = !(*v);
        MarkItemEdited(id);
    ...
    if (it_anim->second.active_text_anim > 120)
        it_anim->second.active_text_anim = 120;
    ...
    window->DrawList->AddRectFilled(rect.Max - ImVec2(50, 50), rect.Max, ImColor(255, 255, 255, 15), 2.f);
    window->DrawList->AddRectFilled(rect.Max - ImVec2(50, 50), rect.Max, ImColor(100, 108, 252, it_anim->second.active_rect_alpha), 2.f);
    window->DrawList->AddRect(rect.Max - ImVec2(50, 50), rect.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 2.f);
    window->DrawList->AddText(rect.Min + ImVec2(57, 15), ImColor(255, 255, 255, (135 + it_anim->second.active_text_anim)), label);
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    const float square_sz = GetFrameHeight() + 75;
    const ImVec2 pos = window->DC.CursorPos;
    ...
    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + 15 + style.FramePadding.y);
    if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max + ImVec2(0, 14), ImColor(0.06f, 0.06f, 0.06f, 1.f), 3.f);

    window->DrawList->AddRect(clickable.Min, clickable.Max + ImVec2(0, 14), ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 3.f);

    window->DrawList->AddText(rect.Min, ImColor(145, 145, 145, 255), label);

    window->DrawList->PushClipRect(clickable.Min, clickable.Max + ImVec2(0, 14) - ImVec2(29, -9));
    window->DrawList->AddText(clickable.Min + ImVec2(5, 7), ImColor(255, 255, 255, 255), preview_value);
    window->DrawList->PopClipRect();

    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 14), 3.f + it_anim->second.active_circle_anim, ImColor(100, 108, 252, 255));
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(5, 10)); // Horizontally align ourselves with the framed text
    PushStyleVar(ImGuiStyleVar_PopupBorderSize, 1.f);
    PushStyleVar(ImGuiStyleVar_PopupRounding, 3.f);
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(11, 11, 11, 255)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() + 70;
    ...
    if (it_anim->second.circle_anim > 0.5f)
        it_anim->second.circle_anim = 0.5f;
    else if (it_anim->second.circle_anim < 0)
        it_anim->second.circle_anim = 0;


    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 5.f), clickable.Max - ImVec2(132.f, -20.f), ImColor(255, 255, 255, 15), 5.0f);
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f,5.f), grab_bb.Max - ImVec2(132.f, -20.f), ImColor(100, 108, 252, 255), 5.0f);
   // window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(0.f, -10.f), 5.f + it_anim->second.circle_anim, ImColor(100, 108, 252, 255), 30.f);
    //window->DrawList->AddText(rect.Min - ImVec2(0, -10), ImColor(255, 255, 255, 255), label);

    //RenderTextClipped(rect.Min - ImVec2(-20, -10), rect.Max, value_buf, value_buf_end, NULL, ImVec2(1.f, 0.f));
...
=======
>>>>>>> parent of 624f49a (fuck hussian)
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(9, 2), sv_cursor_pos + ImVec2(-3, 2), ImColor(0, 0, 0));
    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(8, 1), sv_cursor_pos + ImVec2(-4, 1), ImColor(255, 255, 255));

    draw_list->AddRectFilled(sv_cursor_pos + ImVec2(3, -2), sv_cursor_pos + ImVec2(9, 2), ImColor(0, 0, 0));
    draw_list->AddRectFilled(sv_cursor_pos + ImVec2(4, -1), sv_cursor_pos + ImVec2(8, 1), ImColor(255, 255, 255));

    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(2, 9), sv_cursor_pos + ImVec2(2, -3), ImColor(0, 0, 0));
    draw_list->AddRectFilled(sv_cursor_pos - ImVec2(1, 8), sv_cursor_pos + ImVec2(1, -4), ImColor(255, 255, 255));

    draw_list->AddRectFilled(sv_cursor_pos + ImVec2(-2, 3), sv_cursor_pos + ImVec2(2, 9), ImColor(0, 0, 0));
    draw_list->AddRectFilled(sv_cursor_pos + ImVec2(-1, 4), sv_cursor_pos + ImVec2(1, 8), ImColor(255, 255, 255));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min + ImVec2(0, 2.5f), bb_inner.Max + ImVec2(0, 2.5f), GetColorU32(col_source), rounding);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            RenderFrameBorder(bb.Min + ImVec2(0, 2.5f), bb.Max + ImVec2(0, 2.5f), rounding);
        else
            window->DrawList->AddRect(bb.Min + ImVec2(0, 2.5f), bb.Max + ImVec2(0, 2.5f), ImColor(255, 255, 255, 15), rounding); // Color button are often in need of some sort of border
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->AddText(text_min + ImVec2(0, 1), ImColor(255, 255, 255, 105 + it_anim->second.active_text_anim + it_anim->second.hovered_text_anim), label);
